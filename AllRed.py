...
ATTACK_SPEED = 0.005    # very smooth attack (gradual rise)
CALIBRATION_TIME = 3
NOISE_MARGIN = 1.05
...
RED_TRIGGER = 0.95   # red zone threshold
...
def update_leds(level):
    ...
    # 🔴 If in red zone → full solid red
    if level >= RED_TRIGGER:
        for i in range(NUM_LEDS):
            np[i] = (255, 0, 0)
        np.write()
        return

    # Normal gradient below red zone
    for i in range(NUM_LEDS):
        if i < leds_on:
            frac = i / (NUM_LEDS - 1)
